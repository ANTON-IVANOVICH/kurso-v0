// Fetch client for the kurso-api merchant (partner cabinet) endpoints.
//
...
// `/partner/auth/refresh` once (single-flight) and retries the original request.
...
export function refreshAccess(): Promise<boolean> {
  if (!refreshing) {
    refreshing = raw('POST', '/partner/auth/refresh', undefined, false)
      .then(async (res) => {
        if (!res.ok) {
          accessToken = null
          return false
...
async function request<T>(method: string, path: string, body?: unknown): Promise<T> {
  const isAuthCall = path.startsWith('/partner/auth/')
  let res = await raw(method, path, body)

  // Access expired — refresh once and retry (never for the auth calls themselves).
